...
function updateWeatherUI(data) {
    ...
    if (weatherCondition.includes('rain')) {
        ...
        video = weatherVideos["default"];
    }

    // Log the video source for debugging
    console.log("Setting video source to:", video);
    ...
    weatherVideo.addEventListener('error', () => {
        console.log("Error loading video, setting to default.mp4");
        weatherVideo.src = weatherVideos["default"];
        weatherVideo.play();
    });

    // Add event listener to log when the video is loaded
    weatherVideo.addEventListener('loadeddata', () => {
        console.log("Video loaded successfully:", weatherVideo.src);
        weatherVideo.play();
    });
